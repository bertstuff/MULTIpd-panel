...
PROCESS_THREAD(SCR_Menu_process, ev, data){
	...
	//Setup Start menu
	cur_admin_code = Load_code_from_sd(AUTH_ADMIN);
	cur_Service_code = Load_code_from_sd(AUTH_SERVICE);
	cur_menu->Menu = SCR_get_item_on_page(cur_menu,0,0);
	SCR_build_menu(true);
	...
	if(cur_Service_code != 0){
		Save_code_to_sd(AUTH_SERVICE,cur_Service_code);
	}

	if(cur_admin_code != 0){
		Save_code_to_sd(AUTH_ADMIN,cur_admin_code);
	}
