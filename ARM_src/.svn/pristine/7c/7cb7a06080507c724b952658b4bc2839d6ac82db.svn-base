...
PT_THREAD(RFID_keys_init(void)){
	...
	for(RFID_cur_reader= 0; RFID_cur_reader < RFID_reader_cnt; RFID_cur_reader++){
	   if(RFID_Reader[RFID_cur_reader].reader_type == READER_MIFARE){
		   ...
		   PT_SPAWN(&pt_RFID_sub, &pt_RFID_send, RFID_send_command(&RFID_Reader[RFID_cur_reader], &com_status, RFID_KeyStore, data_byte, 7, 0));
	   }
   }
...
PT_THREAD(RFID_hitag_init(void)){
	...
	for(RFID_cur_reader= 0; RFID_cur_reader < RFID_reader_cnt; RFID_cur_reader++){
	   if(RFID_Reader[RFID_cur_reader].reader_type >= READER_HIGTAG_2){
		   ...
		   data_byte[1] = 0x00;
		   PT_SPAWN(&pt_RFID_sub, &pt_RFID_send, RFID_send_command(&RFID_Reader[RFID_cur_reader], &com_status, RFID_ProgEprom, data_byte, 2, 0));
		   data_byte[0] = Higtag_type_byte(RFID_Reader[RFID_cur_reader].reader_type);
		   PT_SPAWN(&pt_RFID_sub, &pt_RFID_send, RFID_send_command(&RFID_Reader[RFID_cur_reader], &com_status, RFID_ReaderType, data_byte, 1, 0));
	   }
   }
   PT_END(&pt_RFID_sub);
}
...
PT_THREAD(RFID_led_buzzer_state(RFID_reader_t * reader, bool busy)){
	...
	PT_SPAWN(&pt_RFID_sub, &pt_RFID_send, RFID_send_command(reader,&com_status, RFID_OutPut, &inv_output_state, 1, 0));
