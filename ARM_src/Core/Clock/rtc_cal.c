...
void rtc_cal_init(struct tm  *pFullTime)
{
	...
	rtc_cal_settime(pFullTime);
}

void rtc_cal_settime(struct tm  *pFullTime)
{
	RTC_TIME_Type RTC_pFullTime;
	RTC_pFullTime.YEAR = pFullTime->tm_year;
	RTC_pFullTime.MONTH = pFullTime->tm_mon;
	RTC_pFullTime.DOM = pFullTime->tm_mday;
	RTC_pFullTime.DOW = pFullTime->tm_wday;
	RTC_pFullTime.DOY = pFullTime->tm_yday;
	RTC_pFullTime.HOUR = pFullTime->tm_hour;
	RTC_pFullTime.MIN = pFullTime->tm_min;
	RTC_pFullTime.SEC = pFullTime->tm_sec;

	RTC_Cmd(LPC_RTC, DISABLE);
	RTC_SetFullTime(LPC_RTC, &RTC_pFullTime);
...
{
	RTC_TIME_Type RTC_pFullTime;
	RTC_GetFullTime (LPC_RTC,&RTC_pFullTime);

	pFullTime->tm_year = RTC_pFullTime.YEAR;
	pFullTime->tm_mon = RTC_pFullTime.MONTH;
	pFullTime->tm_mday = RTC_pFullTime.DOM;
	pFullTime->tm_wday = RTC_pFullTime.DOW;
	pFullTime->tm_yday = RTC_pFullTime.DOY;
	pFullTime->tm_hour = RTC_pFullTime.HOUR;
	pFullTime->tm_min = RTC_pFullTime.MIN;
	pFullTime->tm_sec = RTC_pFullTime.SEC;
