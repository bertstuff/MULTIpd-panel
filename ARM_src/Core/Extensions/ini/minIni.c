...
#include "minIni.h"
 ...
 const char inifile_machine[] = "SD1:/MACHINE.cfg";
 const char inifile_device[] = "SD1:/DEVICE.cfg";
 const char inifile_net[] = "SD1:/NET.cfg";
#if !defined __T
  #include <ctype.h>
  #include <string.h>
  #include <stdlib.h>
...
__BSS(RAM2) char text_buffer[150];
 ...
 * \param DefValue    default time in the event of a failed read
 * \param Description when not NULL and Key not found save defval and description
 * \param Filename    the name and full path of the .ini file to read from
 *
 * \return            the number of characters copied into the supplied buffer
 */
struct tm ini_getDateTime(const TCHAR *Section, const TCHAR *Key, struct tm DefValue, const TCHAR *Description, const TCHAR *Filename)
{
	TCHAR Buffer[60];
	struct tm time = {.tm_mday = 0,.tm_mon = 0,.tm_year = 0,.tm_hour = 0,.tm_min = 0,.tm_sec = 0};

	TCHAR DefValuestring[60]="";
	TCHAR tmpstring[10]="";

	if((DefValue.tm_mday != 0)||(DefValue.tm_mon != 0)||(DefValue.tm_year != 0)){
		sprintf(tmpstring, "%02d", DefValue.tm_mday);
		strcat ( DefValuestring, tmpstring);
	}

	if((DefValue.tm_mon != 0)||(DefValue.tm_year != 0)){
		sprintf(tmpstring, "-%02d", DefValue.tm_mon + 1);
		strcat ( DefValuestring, tmpstring);
	}

	if(DefValue.tm_year != 0){
		sprintf(tmpstring, "-%04d", DefValue.tm_year + 1900);
		strcat ( DefValuestring, tmpstring);
	}

	if((DefValue.tm_hour != 0)||(DefValue.tm_min != 0)||(DefValue.tm_sec != 0)){
		sprintf(tmpstring, " %02d",DefValue.tm_hour);
		strcat ( DefValuestring, tmpstring);
	}

	if((DefValue.tm_min != 0)||(DefValue.tm_sec != 0)){
		sprintf(tmpstring, ":%02d",DefValue.tm_min);
		strcat ( DefValuestring, tmpstring);
	}

	if(DefValue.tm_sec != 0){
		sprintf(tmpstring, ":%02d",DefValue.tm_sec);
		strcat ( DefValuestring, tmpstring);
	}

	ini_gets(Section, Key, DefValuestring, Description, Buffer, sizeof(Buffer), Filename);

	time = string_to_time(Buffer);
	return time;
}
 ...
 * \return            the number of characters copied into the supplied buffer
 */
int ini_gets(const TCHAR *Section, const TCHAR *Key, const TCHAR *DefValue, const TCHAR *Description,
             TCHAR *Buffer, int BufferSize, const TCHAR *Filename)
{
  ...
  if (!ok){
	  printf("INI: Cant find %s [%s] %s set to default (%s)\r\n",Filename,Section,Key,DefValue);
     save_strncpy(Buffer, DefValue, BufferSize, QUOTE_NONE);
     if(Description != NULL){
		 ini_puts(Section, Key, DefValue, Description, Filename);
     }
  }
  return _tcslen(Buffer);
}

char * ini_string_export(char * dest,const char * source){
	char * dp;
	char * sp;

	dp = dest;
	for(sp = (char *)source; *sp != '\0'; sp++){
		if(*sp == '\n'){
			*dp++ = '\\';
			*dp++ = 'n';
		}else if(*sp == '\r'){
...
char * ini_string_import(char * dest,char * source){
	char * dp;
	char * sp;
...
/*
 * @brief get some memory space to return text.
 * @param length memory needed (max 100)
 */
char * get_textbuffer(uint8_t length){
	static uint8_t buffer_place = 0;
	TCHAR * text_ptr = NULL;

	if((buffer_place + (length + 1)) >= sizeof(text_buffer)){
		buffer_place = 0;
	}
	text_ptr = &text_buffer[buffer_place];
	buffer_place += (length + 1);
	return text_ptr;
}
#if(BOARD_SCREEN == 1)
char * ini_text(const TCHAR * Text_ref, Language_t language, Screen_t Screen_type){
		  TCHAR Filename[25] = "SD1:/TEXT_";
		  const TCHAR * Section[4]={"NL","EN","DE","FR"};
		  const char * Def_text = "????";
		  TCHAR * buffer_ptr = NULL;
		  TCHAR * text_ptr = NULL;
		  char local_text_buffer[150];
		  ...
		  if (Text_ref == NULL)
			return 0;
			if (ini_openread(Filename, &fp)) {
				ok = getkeystring(&fp, Section[language], Text_ref, -1, -1, (TCHAR *)local_text_buffer, 250);
				quotes = QUOTE_DEQUOTE;
				buffer_ptr = skipleading(local_text_buffer);
				cleanstring(buffer_ptr,&quotes);
				text_ptr = get_textbuffer(strlen(buffer_ptr));
				ini_string_import(text_ptr,buffer_ptr);
				(void)ini_close(&fp);
			} /* if */

		  if (!ok){
			 ini_puts("LANG", Text_ref, ini_string_export(text_buffer,Def_text), NULL, "SD1:/TEXT.ini");
			 text_ptr = get_textbuffer(strlen(Def_text));
			 save_strncpy(text_ptr, Def_text, 100, QUOTE_NONE);
		  }
		  return text_ptr;
	}
#endif
 ...
 */
long ini_getl(const TCHAR *Section, const TCHAR *Key, long DefValue, const TCHAR *Description, const TCHAR *Filename)
{
  TCHAR LocalBuffer[64];
  long2str(DefValue, LocalBuffer);
  int len = ini_gets(Section, Key, LocalBuffer, Description, LocalBuffer, sizearray(LocalBuffer), Filename);
 ...
 * \return            the value located at Key
 */
INI_REAL ini_getf(const TCHAR *Section, const TCHAR *Key, INI_REAL DefValue, const TCHAR *Description, const TCHAR *Filename)
{
  TCHAR LocalBuffer[64];
  ini_ftoa(LocalBuffer, DefValue);
  int len = ini_gets(Section, Key, LocalBuffer, Description, LocalBuffer, sizearray(LocalBuffer), Filename);
 ...
 */
int ini_getbool(const TCHAR *Section, const TCHAR *Key, int DefValue, const TCHAR *Description, const TCHAR *Filename)
{
  TCHAR LocalBuffer[2];
  int ret;
  ini_gets(Section, Key, DefValue?"1":"0", Description, LocalBuffer, sizearray(LocalBuffer), Filename);
...
static void writekey(TCHAR *LocalBuffer, const TCHAR *Key, const TCHAR *Value, const TCHAR *Description, INI_FILETYPE *fp)
{
  TCHAR *p;
  enum quote_option option = check_enquote(Value);
  if(Description != NULL){
	  *LocalBuffer = ';';
	  save_strncpy(LocalBuffer+1, Description, INI_BUFFERSIZE - 4, QUOTE_NONE);
	  p = _tcsrchr(LocalBuffer, '\0');
	  assert(p != NULL);
	  *p++ = '\r';
	  *p++ = '\n';
  ...
  save_strncpy(p, Key, INI_BUFFERSIZE - (p - LocalBuffer) - 3, QUOTE_NONE);  /* -1 for '=', -2 for '\r\n' */
  p = _tcsrchr(LocalBuffer, '\0');
  assert(p != NULL);
  *p++ = '=';
 ...
 * \param Value       a pointer to the time, or NULL to erase the key
 * \param Filename    the name and full path of the .ini file to write to
 *
 * \return            1 if successful, otherwise 0
 */
int ini_putDateTime(const TCHAR *Section, const TCHAR *Key, struct tm *Value, const TCHAR *Description, const TCHAR *Filename){
	char buffer[25];

	if(Value == NULL){
		return ini_puts(Section, Key, NULL, Description, Filename);
	}
	...
	return ini_puts(Section, Key, buffer, Description, Filename);
}
 ...
 * \return            1 if successful, otherwise 0
 */
int ini_puts(const TCHAR *Section, const TCHAR *Key, const TCHAR *Value, const TCHAR *Description, const TCHAR *Filename)
{
  INI_FILETYPE rfp;
  INI_FILETYPE wfp;
  INI_FILEPOS mark;
  ...
  rs = f_open(&rfp, Filename, FA_READ+FA_OPEN_EXISTING);
  ...
  if (rs == FR_NO_FILE) {
    ...
    if (Key != NULL && Value != NULL) {
      ...
      writekey(LocalBuffer, Key, Value, Description, &wfp);
      (void)ini_close(&wfp);
    } /* if */
    return 1;
  }else if(rs != FR_OK){
      ...
      if (!ini_read(LocalBuffer, INI_BUFFERSIZE, &rfp)) {
    	  if(ini_eof(&rfp)){
			...
			return close_rename(&rfp, &wfp, Filename, LocalBuffer);  /* clean up and rename */
    		  ...
    		  (void)ini_close(&wfp);
    		  (void)ini_close(&rfp);
  			  (void)ini_tempname(LocalBuffer, Filename, INI_BUFFERSIZE);
  ...
  cache_flush(LocalBuffer, &cachelen, &rfp, &wfp, &mark);
  if (Key != NULL && Value != NULL)
    writekey(LocalBuffer, Key, Value, (flag)?Description:NULL, &wfp);
 ...
 * \return            1 if successful, otherwise 0
 */
int ini_putl(const TCHAR *Section, const TCHAR *Key, long Value, const TCHAR *Description, const TCHAR *Filename)
{
  TCHAR LocalBuffer[32];
  long2str(Value, LocalBuffer);
  return ini_puts(Section, Key, LocalBuffer, Description, Filename);
}
 ...
 * \return            1 if successful, otherwise 0
 */
int ini_putf(const TCHAR *Section, const TCHAR *Key, INI_REAL Value, const TCHAR *Description, const TCHAR *Filename)
{
  TCHAR LocalBuffer[64];
