	...
	if (node->value.element.num_attrs)
	{
	  for (i = 0; i < node->value.element.num_attrs; i ++)
	  {
	    if (node->value.element.attrs[i].name)
	    	m_free(node->value.element.attrs[i].name);
	    if (node->value.element.attrs[i].value)
	    	m_free(node->value.element.attrs[i].value);
	  }

	  m_free(node->value.element.attrs);
	}
        break;
    case MXML_INTEGER :
       /* Nothing to do */
         ...
         mxml_type_t type)		/* I - Node type */
{
  ...
  */
  ...
  if ((node = m_calloc(1, sizeof(mxml_node_t))) == NULL)
  {
#if DEBUG > 1
