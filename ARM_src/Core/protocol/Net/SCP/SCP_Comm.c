...
SCP_error_t g_SCP_error = E_DISABLED;
...
void SCP_Init(GroupId_t groupnr, bool device_reset){
	...
	SCP_Get_device_info();
	if(ini_getbool("Server Communication", "SCP_enable",true,"Enable SCP:", inifile_net) == false){
		return;
	}
...
short unsigned int SCP_packet_to_data(void * packet){
	...
	snprintf(packet_buffer,150,"%d %d %d %x %x %x %x %x %x %x %d %d %d %d %s%s%s",
	msg_id,
	g_DevInfo.SCP_devID,
...
PROCESS_THREAD(SCP_process, ev, data){
	...
	while(1){
		...
		if(ev == PROCESS_EVENT_TIMER){
			...
			if((Device_loop != NULL) && (!Save_reset_active())){
...
PROCESS_THREAD(SCP_devInfo_process, ev, data){
	...
	PROCESS_BEGIN();
	etimer_set(&Module_DevInfo_tm, CLOCK_SECOND);
	PROCESS_WAIT_UNTIL(Net_settings.setup_done == true);
		...
		if(ev == PROCESS_EVENT_TIMER){
			...
			if(etimer_expired(&Module_DevInfo_tm)){
				...
				if(SCP_packet->Error == E_NONE){
					...
					if(Device_loop->Info.SCP_devID == 0){
						etimer_set(&DevInfo_check_tm, CLOCK_SECOND * (DevInfo_interval * 3));
					}
